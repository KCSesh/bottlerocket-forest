# Feature 0003: Sembly CLI - Technical Design

## Overview

The Sembly CLI provides a command-line interface for semantic search and knowledge indexing. It wraps sembly-core functionality with user-friendly commands, progress reporting, and formatted output.

Design Philosophy: The CLI is a thin presentation layer over sembly-core. Commands map directly to domain operations with minimal transformation. The CLI handles argument parsing, user feedback, and output formatting—domain logic lives in the core library.
...
Command handlers (index/, context/, gc.rs)
  ↓
sembly-core (KnowledgeIndex facade)
  ↓
Domain operations
...
The CLI operates on these core concepts from sembly-core:
...
- Creates .sembly/knowledge/ directory
...
IndexError - Wraps sembly_core::knowledge::facade::IndexError
...
Alternatives considered:
1. sembly context <name> search <query> (context as parent command)
2. sembly search <query> --context <name> (context as flag, chosen)
...
- Keep handlers thin—delegate to sembly-core immediately
...
- CLI version should match sembly-core version
