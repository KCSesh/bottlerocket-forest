# Sembly Core - Technical Design

## Overview

Sembly Core implements semantic search for documentation using a layered architecture with type-safe domain models at its core. The design prioritizes testability, modularity, and performance while keeping the embedding model and vector database self-contained.
...
Indexing Layer (indexing/) - File scanning and index building. FileScanner traverses directories respecting .gitignore and .semblyignore. Indexer orchestrates the indexing workflow: scan → chunk → embed → store. Supports incremental updates via modification time tracking. Context-aware indexing for multi-context support.
...
- sembly search "query" --context worktrees/feature-a searches within that context
...
SemblyConfig
- Configuration file (.sembly.toml) for customizing indexing
- Properties: include/exclude patterns, boost rules, chunking settings
- Loaded via load_sembly_config()
...
- with_config(config: SemblyConfig) -> Self
...
│   ├── indexer.rs            # Indexer
│   ├── config.rs             # SemblyConfig, load_sembly_config()
