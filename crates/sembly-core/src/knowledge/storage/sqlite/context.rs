...
mod test {
    ...
    fn make_context(id: &str) -> Context {
        Context {
    ...
    fn insert_and_retrieve_context() {
        ...
        let context = make_context("my-project");
    ...
    fn list_multiple_contexts() {
        ...
        let conn = setup_connection();
        let repo = SqliteContextRepository::new(&conn);
        let ctx1 = make_context("project-a");
        let ctx2 = make_context("project-b");
        let ctx3 = make_context("worktrees/feature-x");

        repo.insert_context(&ctx1).unwrap();
        repo.insert_context(&ctx2).unwrap();
        repo.insert_context(&ctx3).unwrap();
    ...
    fn remove_context() {
        ...
        let conn = setup_connection();
        let repo = SqliteContextRepository::new(&conn);
        let context = make_context("to-remove");
        repo.insert_context(&context).unwrap();
    ...
    fn duplicate_insert_fails_with_already_exists() {
        ...
        let conn = setup_connection();
        let repo = SqliteContextRepository::new(&conn);
        let context = make_context("duplicate");
        repo.insert_context(&context).unwrap();
