...
pub enum SearchError {
    ...
    #[diagnostic(
        code(sembly::search::storage_error),
    ...
    #[diagnostic(
        code(sembly::search::embedding_failed),
    ...
    #[diagnostic(
        code(sembly::search::invalid_score),
        help("The index may be corrupted. Try running `sembly rebuild`")
    )]
    InvalidScore {
        score: f32,
        source: Box<dyn std::error::Error + Send + Sync + 'static>,
