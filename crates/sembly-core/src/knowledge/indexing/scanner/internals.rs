...
impl FileScanner {
    ...
    fn extract_repo_name(&self, path: &Path) -> Result<RepoName, ScanError> {
        let relative =
            path.strip_prefix(&self.forest_root)
    ...
    fn create_paths(&self, path: &Path) -> Result<(AbsolutePath, ForestRelativePath), ScanError> {
        ...
        let relative_path =
            ForestRelativePath::try_new(relative.display().to_string()).map_err(|e| {
                ScanError::PathCreation {
                    source: Box::new(e) as Box<dyn std::error::Error + Send + Sync>,
                }
            })?;
