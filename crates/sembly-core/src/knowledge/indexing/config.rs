...
impl SemblyConfig {
    ...
    pub fn to_indexing_filter(&self) -> Result<IndexingFilter, SemblyConfigError> {
...
impl RustConfig {
    ...
    fn to_rust_filter(&self) -> Result<RustFilter, SemblyConfigError> {
        Ok(RustFilter::new(
...
pub fn load_sembly_config(
...
) -> Result<Option<SemblyConfig>, SemblyConfigError> {
    ...
    let config: SemblyConfig = toml::from_str(&content).context(ParseSnafu)?;
    ...
    for target in &config.targets {
        if target.is_absolute() {
            return Err(SemblyConfigError::InvalidPath {
        ...
        if let Ok(canonical) = target_path.canonicalize() {
            if !canonical.starts_with(&forest_root_canonical) {
                return Err(SemblyConfigError::PathEscapesRoot {
                    path: target.display().to_string(),
                });
            }
...
/// Errors that can occur when loading sembly configuration
#[derive(Debug, Snafu)]
#[snafu(module)]
#[non_exhaustive]
pub enum SemblyConfigError {
...
"#;
    ...
    #[test]
    fn test_load_sembly_config_with_invalid_toml() {
        ...
        assert!(matches!(result, Err(SemblyConfigError::ParseError { .. })));
    }
...
"#;
        ...
        assert!(matches!(
            result,
            Err(SemblyConfigError::PathEscapesRoot { .. })
        ));
    }
...
"#;
    ...
    #[test]
    fn test_sembly_config_to_indexing_filter() {
        // Given A config with Rust filtering
        let config = SemblyConfig {
