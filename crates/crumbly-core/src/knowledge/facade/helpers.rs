...
impl KnowledgeIndex {
    ...
    ///
    /// Returns `<index_root>/.crumbly/knowledge.db`
    ...
    /// and score boosting rules from the crumbly configuration.
    ...
    pub(super) fn create_embedding_model(&self) -> Result<LoadedEmbeddingModel, IndexError> {
        ...
        EmbeddingModel::builder()
            ...
            .cache_dir(self.index_root.join(SEMBLY_DIR).join(MODEL_CACHE_DIR))
            .build()
            .load()
            .context(EmbeddingProviderCreationFailedSnafu)
    }

    /// Load score booster from crumbly configuration
    ...
    pub(super) fn load_scan_config_for_context(
        ...
        let crumbly_config =
            indexing::load_crumbly_config(&self.index_root).context(ConfigLoadFailedSnafu)?;
    ...
    /// Load indexing filter rules from crumbly configuration
    ///
    /// Reads `.crumbly.toml` to determine which files should be excluded from indexing.
