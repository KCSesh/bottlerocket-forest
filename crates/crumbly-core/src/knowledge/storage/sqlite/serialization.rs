...
/// Helper to convert domain type construction errors to StorageError
fn field_error<T, E>(result: Result<T, E>, field: &str) -> Result<T, StorageError>
where
...
pub fn indexed_chunk_from_row(row: &rusqlite::Row) -> Result<IndexedChunk, StorageError> {
    ...
    let embedding = field_error(Embedding::try_new(vec![0.1; EMBEDDING_DIM]), "embedding")?;
    ...
    let chunk = Chunk::builder()
        ...
        .source(
            ChunkSource::builder()
                .file_path(field_error(
                    IndexRelativePath::try_new(file_path_value),
                    "file_path",
                )?)
                .repo_name(field_error(RepoName::try_new(repo_name), "repo_name")?)
                .build(),
        )
        .content(
            ChunkContent::builder()
                .text(content)
                .token_count(field_error(
