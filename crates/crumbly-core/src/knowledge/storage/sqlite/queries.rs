...
///
/// Returns all files tracked in the specified context.
pub fn get_indexed_files(
    conn: &Connection,
    context_id: &ContextId,
) -> Result<std::collections::HashMap<IndexRelativePath, Timestamp>, StorageError> {
    ...
    for row in rows {
        ...
        if let Ok(path) = IndexRelativePath::try_new(&file_path) {
...
/// Helper function to retrieve and parse metadata values
fn get_metadata_value<T: std::str::FromStr>(
    conn: &Connection,
    key: &str,
    default: T,
...
pub fn get_metadata(conn: &Connection) -> Result<IndexMetadata, StorageError> {
    let last_build_unix: i64 = get_metadata_value(conn, "last_build", 0)?;
    ...
    let model_name: String = get_metadata_value(
        conn,
        "model_name",
        crate::knowledge::constants::DEFAULT_TOKENIZER_MODEL.to_string(),
    )?;

    let embedding_dim: usize = get_metadata_value(
        conn,
        "embedding_dim",
        crate::knowledge::constants::EMBEDDING_DIM,
    )?;

    let max_tokens: usize = get_metadata_value(
        conn,
        "max_tokens",
        crate::knowledge::constants::DEFAULT_MAX_CHUNK_TOKENS,
    )?;

    let overlap_tokens: usize = get_metadata_value(
        conn,
        "overlap_tokens",
        crate::knowledge::constants::DEFAULT_CHUNK_OVERLAP_TOKENS,
    )?;
