...
use crate::knowledge::domain::{Chunk, IndexRelativePath};
...
impl BoostPattern {
    ...
    fn matches(&self, path: &IndexRelativePath) -> bool {
...
pub fn default_boost_rules() -> Vec<BoostRule> {
    vec![
        BoostRule::builder()
            ...
            .pattern(BoostPattern::new("**/README.md").expect("valid glob"))
            .multiplier(BoostMultiplier::try_new(1.2).expect("1.2 in range"))
            .build(),
        BoostRule::builder()
            .description("Documentation directory")
            .pattern(BoostPattern::new("**/docs/**").expect("valid glob"))
            .multiplier(BoostMultiplier::try_new(1.2).expect("1.2 in range"))
            .build(),
        BoostRule::builder()
            .description("Changelogs")
            .pattern(BoostPattern::new("**/CHANGELOG.md").expect("valid glob"))
            .multiplier(BoostMultiplier::try_new(0.8).expect("0.8 in range"))
            .build(),
        BoostRule::builder()
            .description("Markdown documentation files")
            .pattern(BoostPattern::new("**/*.md").expect("valid glob"))
            .multiplier(BoostMultiplier::try_new(1.1).expect("1.1 in range"))
            .build(),
    ]
}
...
mod test {
    ...
    use crate::knowledge::domain::{
        Chunk, ChunkContent, ChunkContext, ChunkHash, ChunkId, ChunkSource, FileHash, RepoName,
        TokenCount,
    };
