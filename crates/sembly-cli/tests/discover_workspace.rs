...
//!
//! Tests that sembly can find the .sembly database by walking up parent directories.
//!
//! Run with: `cargo test --test discover_workspace -- --ignored`

mod common;

use common::{sembly_build, sembly_cmd, setup_fixture};
use std::path::Path;

fn sembly_cmd_from_dir(workspace: &Path, subdir: &str, args: &[&str]) -> (i32, String, String) {
    let cwd = workspace.join(subdir);
    let mut cmd = std::process::Command::new(env!("CARGO_BIN_EXE_sembly"));
    cmd.current_dir(&cwd);
    for arg in args {
        cmd.arg(arg);
    }
    let output = cmd.output().expect("Failed to execute sembly");
...
fn discover_status_from_subdirectory() {
    ...
    let (code, stdout, stderr) = sembly_cmd_from_dir(workspace.path(), "src", &["status"]);
...
fn discover_search_from_subdirectory() {
    ...
    let (code, stdout, stderr) = sembly_cmd_from_dir(workspace.path(), "src", &["search", "startup"]);
...
fn discover_fails_without_database() {
    ...
    let (code, _, stderr) = sembly_cmd_from_dir(workspace.path(), "subdir", &["status"]);
