...
use common::{
    sembly_build, sembly_build_context, sembly_cmd, sembly_search_context,
    sembly_search_with_chunks, sembly_status_chunk_count, sembly_update_context, setup_fixture,
};
...
fn context_multiple_contexts_can_be_indexed() {
    ...
    let (code, stdout, _) =
        sembly_search_context(workspace.path(), "database connection handler", "main");
    ...
    let (code, stdout, _) =
        sembly_search_context(workspace.path(), "database connection handler", "worktree");
...
fn context_list_shows_registered_contexts() {
    ...
    let (code, stdout, _) = sembly_cmd(workspace.path(), &["context", "list"]);
...
fn context_default_build_creates_dot_context() {
    ...
    // When: Running sembly build without --context
    ...
    // When: Running sembly build again without --context
    let (code, _, stderr) = sembly_build(workspace.path());
...
fn context_remove_deletes_context() {
    ...
    let (_, stdout, _) = sembly_cmd(workspace.path(), &["context", "list"]);
    assert!(stdout.contains("main") && stdout.contains("worktree"));

    // When: Removing the worktree context
    let (code, _, stderr) = sembly_cmd(workspace.path(), &["context", "remove", "worktree"]);
...
fn context_remove_nonexistent_fails() {
    ...
    let (code, _, _) = sembly_cmd(workspace.path(), &["context", "remove", "nonexistent"]);
...
fn context_remove_default_fails() {
    ...
    let (code, _, stderr) = sembly_cmd(workspace.path(), &["context", "remove", "."]);
...
fn gc_removes_unreferenced_embeddings() {
    ...
    let chunks_before_remove = sembly_status_chunk_count(workspace.path());
    ...
    let chunks_after_remove = sembly_status_chunk_count(workspace.path());
    assert_eq!(
    ...
    let (code, stdout, stderr) = sembly_cmd(workspace.path(), &["gc"]);
    assert_eq!(code, 0, "GC failed: stdout={}\nstderr={}", stdout, stderr);

    // Then: Chunk count should decrease (orphans removed)
    let chunks_after_gc = sembly_status_chunk_count(workspace.path());
    assert!(
...
fn context_clear_removes_file_mappings() {
    ...
    let (code, stdout, _) = sembly_search_with_chunks(workspace.path(), "startup initialization");
    assert_eq!(code, 0, "Search failed");
    assert!(
    ...
    // When: Running sembly clear
    let (code, _, stderr) = sembly_cmd(workspace.path(), &["clear"]);
...
"#;
    ...
    // And: Running sembly update for main context only
    let (code, stdout, stderr) = sembly_update_context(workspace.path(), "main");
    assert_eq!(
        code, 0,
    ...
    let (code, stdout, _) =
        sembly_search_context(workspace.path(), "zebra unicorn rainbow", "main");
    assert_eq!(code, 0, "Search failed");
    assert!(
    ...
    let (code, stdout, _) =
        sembly_search_context(workspace.path(), "zebra unicorn rainbow", "worktree");
